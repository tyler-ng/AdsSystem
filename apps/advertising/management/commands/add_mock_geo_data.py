...
# Device types for mock data
DEVICE_TYPES = ['smartphone', 'tablet', 'desktop']

# OS types for mock data
OS_TYPES = ['iOS', 'Android', 'Windows', 'macOS']


class Command(BaseCommand):
    help = 'Adds mock geographic distribution data to test campaigns'
    ...
    def handle(self, *args, **options):
        campaign_name = options.get('campaign', 'Test Campaign')
        ...
        if should_reset:
            ...
            self.stdout.write(self.style.WARNING(f'Deleted existing impression data for campaign: {campaign_name}'))
        ...
        total_created = sum(count for _, count in COUNTRIES_DATA)
        self.stdout.write(
            self.style.SUCCESS(
                f'Successfully added {total_created} mock geographic impressions for {len(COUNTRIES_DATA)} countries'
            )
        ) 